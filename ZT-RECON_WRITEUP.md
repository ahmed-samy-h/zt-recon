...
- **ASCII Banner Font *(اتغيّر في v2.1.0)*:** خط الـ ASCII banner اللي بيطبع اسم الأداة أول ما تشتغل اتغيّر لـ `Big Money-ne`، بيتنزّل من مصدر خارجي (`raw.githubusercontent.com/xero/figlet-fonts`) لو مش موجود جوه مكتبة `pyfiglet` أصلاً. لسه بنفس الأسلوب البصري الأحمر بتاع "Zero Trace".
  - **Download Caching:** لو محاولة تنزيل الخط فشلت (404، مفيش نت، DNS مش شغال، إلخ)، الأداة بتحط ملف marker مؤقت (`/tmp/.zt_recon_font_unavailable`) وترجع فورًا لخط احتياطي (`slant`/`big`/`standard`) — وده عشان أي تشغيلة جاية تتخطى محاولة التنزيل خالص من غير ما تحاول وتفشل تاني في كل مرة، بدل ما رسالة "Network Error" تطبع في كل مرة تشغّل فيها الأداة.
...
>
> **تحديث v2.1.0 (الحالي):** الأداة اتنقلت تاني، هذه المرة من Anthropic Claude لـ **Google AI Studio (Gemini API)** عبر مكتبة `google-genai` الرسمية. السبب الأساسي: Google AI Studio بيوفر **free tier فعلي من غير كارت ائتمان**، وكمان **context window أكبر بكتير** بيستحمل براحة البرومبت الكبير اللي الأداة بتبعته (نتائج nmap + subdomains + web + SQLMap/Dirsearch/Nuclei مع بعض في رسالة واحدة)، خصوصًا مع `--full-scan` أو أهداف فيها subdomains كتير. الموديل الافتراضي دلوقتي هو **`gemini-3.5-flash`** (متاح على الـ free tier، توازن قوي بين الجودة والسرعة). تقدر تستخدم `--model gemini-pro-latest` لو محتاج عمق تحليل أكبر (الموديل ده مش متاح على الـ free tier، محتاج billing مفعّل)، أو `--model gemini-flash-lite-latest` للفحوصات السريعة/الـ bulk scans الكبيرة (أعلى rate limit على الـ free tier). زي ما كان الحال مع Anthropic، الـ instructions لسه متفصلة في system instruction منفصل بدل حشرها في رسالة الـ user، والـ temperature لسه `0.2` عشان يقلل من احتمالية إن الموديل "يهلوس" ثغرات أو تفاصيل مش موجودة فعليًا في بيانات الفحص.
>
> **🔁 تحديث إضافي (Retry Logic):** لو استدعاء الـ API فشل، الأداة بقت بتفرّق بين نوعين من الأخطاء: أخطاء شبكة/DNS مؤقتة (زي `Temporary failure in name resolution`، `connection reset`، أو `timeout`) — دي بيتم إعادة المحاولة معاها تلقائيًا لحد **3 مرات** بـ exponential backoff (2 ثانية، 4 ثوان، 8 ثوان)، وأخطاء دائمة (مفتاح غلط/منتهي، اسم موديل غير موجود، quota خلصت) — دي بيتوقف عندها فورًا من غير أي إعادة محاولة عشان معادش فايدة إنها تتكرر. لو كل المحاولات فشلت، عندئذ بس بيترجع نص الخطأ ويتحول لقسم "⚠️ AI Analysis Unavailable" في التقرير زي ما هو موضح تحت.
>
...
- **Streaming حي في التيرمينال** (Markdown منسق برموز 🚨🌐🕳️🛠️📋🔒) — بيتعطّل تلقائيًا وقت الـ parallel bulk scanning (`--threads > 1`)، وكمان بيتعطّل لو اخترت `--report-format` بقيمة `html` أو `pdf` (يعني أي حاجة غير الافتراضي `both`)، عشان مايحصلش تداخل بين مخرجات أكتر من هدف في نفس الوقت أو استهلاك وقت في طباعة مش محتاجها. مع `both` (الافتراضي) وthread واحد، الـ streaming شغّال زي ما هو.
...
- **ملف PDF *(جديد بالكامل v2.0.0)*** — نفس تصميم الـ HTML بالظبط، بيتولّد تلقائيًا جنب كل تقرير HTML عبر مكتبة `weasyprint`.

> **📄 تحديث v2.1.0 — `--report-format` بدل `--no-pdf`:** الفلاج القديم `--no-pdf` **اتشال خالص**. دلوقتي بتتحكم في صيغة/صيغ التقرير اللي هتتحفظ فعليًا عن طريق `--report-format {html,pdf,both}` (الافتراضي `both`). لازم تعرف إن `pdf_report.py` (WeasyPrint) بيرندر الـ PDF **من ملف HTML على الديسك**، يعني ملف الـ HTML دايمًا بيتولّد الأول جوّانيًا مهما كان اختيارك — لو اخترت `pdf` بس، ملف الـ HTML ده بيتعامل معه كـ intermediate مؤقت وبيتمسح تلقائيًا بعد ما الـ PDF يتولّد منه، عشان مايفضلش موجود كملف زيادة مش طالبه في `--report-dir`.
...
- **Selective Scanning:** `--no-subdomains`، `--no-owasp`، و `--report-format` *(اتغيّر من `--no-pdf` في v2.1.0)* لتعطيل أي مرحلة أو للتحكم في مخرجات التقرير مش محتاجها في فحص معين.
...
3. انسخ المفتاح واحتفظ بيه.

> ℹ️ **ملحوظة عن صيغ المفتاح (`AIzaSy...` مقابل `AQ....`):** جوجل بدأت تطرح صيغة جديدة للمفاتيح بادئة بـ `AQ.` جنب الصيغة القديمة `AIzaSy...`. الاتنين شغالين تمام مع الأداة دي، لأنها بتكلم Gemini مباشرة عن طريق مكتبة `google-genai` الرسمية (native SDK). التقارير اللي بتقول إن مفاتيح `AQ.` "مش شغالة" غالبًا بتخص طبقات wrapper بتحاكي OpenAI أو أدوات تالتة بتعمل تحقق (regex) على شكل المفتاح مفترضة إنه لازم يبدأ بـ `AIzaSy` — وده مش اللي ZT-RECON بيعمله. `auth.py` بقى معدّل ومابقاش بيطبع أي تحذير غلط بخصوص مفاتيح `AQ.` (نسخة قديمة من الكود كانت غلط بتحذّر إنها "معطوبة").
...
- **`--model`**
  بتحدد أي موديل من موديلات Google AI Studio (Gemini) يتستخدم في مرحلة التحليل الذكي (AI Analysis). القيمة الافتراضية هي `gemini-3.5-flash` (متاح على الـ free tier، توازن قوي بين الجودة والسرعة). البدائل المتاحة:
...
- **`--report-format` *(بدّل `--no-pdf` في v2.1.0)***
  بتحدد صيغة/صيغ التقرير اللي هتتحفظ فعليًا في `--report-dir`: `html` (HTML بس)، `pdf` (PDF بس)، أو `both` (الافتراضي — الاتنين). لازم تعرف إن ملف HTML بيتولّد دايمًا جوّانيًا الأول (لأن WeasyPrint بيرندر الـ PDF منه)؛ لو اخترت `pdf` بس، ملف الـ HTML ده بيتحذف تلقائيًا بعد ما الـ PDF يتولّد منه عشان يفضل بس الملف اللي طلبته. كمان لو اخترت `html` أو `pdf` (يعني أي حاجة غير `both`)، الـ streaming الحي في التيرمينال بيتعطّل تلقائيًا (النص بيتكتب في ملف/ملفات التقرير بس).
  مثال: `sudo zt-recon -t example.com --report-format html` أو `sudo zt-recon -t example.com --report-format pdf`
...
# تخطي فحص الـ Subdomains أو ترسانة OWASP، والاكتفاء بتقرير HTML بس (بدون PDF)
...
> ⚠️ **ملحوظة مهمة لأول تحديث لـ v2.1.0 تحديدًا:** الأداة اتنقلت من Anthropic Claude لـ Google AI Studio (Gemini)، واسم ملف حفظ المفتاح نفسه اتغيّر (`.anthropic_api_key` → `.google_api_key`). يعني أول تشغيل بعد التحديث، الأداة **هتطلب منك مفتاح Google AI Studio جديد** (مجاني بالكامل من aistudio.google.com) حتى لو كان عندك مفتاح Anthropic قديم متخزن قبل كده — ده سلوك متوقع ومش خطأ، ومفتاح Anthropic القديم بيفضل موجود على الجهاز من غير استخدام، تقدر تمسحه يدويًا لو حابب. (المفتاح الجديد ممكن يبدأ بـ `AIzaSy` أو `AQ.` — الاتنين شغالين، راجع الملحوظة في قسم "الحصول على API Key" فوق.)
